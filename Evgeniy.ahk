...
; http://github.com/FrozenBrain
; Modified by Suchty112
; http://github.com/Suchty112
; Modified by: paul-phoenix
; http://github.com/paul-phoenix
; Modified by: Agrippa1994
; http://github.com/agrippa1994
...
; # 														                                                                    #
; # SAMP-Funktionen:                                                                                                            #
; #     - IsSAMPAvailable()                         PrA?ft, ob man in den Chat schreiben kann & ob GTA geladen ist              #
; #     - isInChat()                                PrA?ft, ob der Spieler gerade chattet oder in einem Dialog ist              #
...
; #     - addChatMessage(wText)                     FA?gt eine Zeile in den Chat ein (nur fA?r den Spieler sichtbar)            #
...
; # --------------------------------------------------------------------------------------------------------------------------- #
; #	- isDialogOpen() - Pruft, ob gerade ein Dialog angezeigt wird (gibt true oder false zuruck)	                        		#
...
; #	- getDialogIndex() - Liest die (zuletzt) ausgewahlte Zeile des Dialogs aus 				                        			#
; #	- getDialogCaption() - Liest die Uberschrift des (zuletzt) angezeigten Dialogs aus 			                        		#
...
; #	- getDialogLines() - Liest die Zeilen mittels getDialogText aus (gibt ein Array zuruck)			                        	#
; #	- isDialogButton1Selected() - Pruft, ob Button1 des Dialogs ausgewahlt ist 						                        	#
...
; #     - getStreamedInPlayersInfo()                Zeigt Informationen A?ber die gestreamten Spieler.                           #
; #     - callFuncForAllStreamedInPlayers()         FA?hrt bestimmte Funktionen, fA?r die gestreamten Spieler aus.                #
...
; #     - getPlayerArmour()                         Ermittelt den RA?stungswert des Spielers                                     #
...
; #	    - getPlayerState()			                Ermittelt den "Status" des Spielers (Zu FuAY, Fahrer, Tot)                   #
...
; #	    - getPlayerPos(X,Y,Z) 			            siehe oben drA?ber 						                                    #
; # 														                                                                    #
; # --------------------------------------------------------------------------------------------------------------------------- #
; # 														                                                                    #
; #     - initZonesAndCities()                      Initialisiert eine Liste aller Standartgebiete                              #
; #                                                 (Voraussetzung fA?r die folgenden Funktionen dieser Kategorie)               #
...
; #     - AddZone(Name, X1, Y1, Z1, X2, Y2, Z2)     FA?gt eine Zone zum Index hinzu                                              #
; #     - AddCity(Name, X1, Y1, Z1, X2, Y2, Z2)     FA?gt eine Stadt zum Index hinzu                                             #
; #	    - IsPlayerInRangeOfPoint(X, Y, Z, Radius)   Bestimmt ob der Spieler in der NAŨe der Koordinaten ist                     #
; #	    - IsIsPlayerInRangeOfPoint2D(X, Y, Radius)  Bestimmt ob der Spieler in der NAŨe der Koordinaten ist                     #
; #	    - getPlayerZone()                   				                                                                    #
; #	    - getPlayerCity()                   					                                                                #
; # 														                                                                    #
; ###############################################################################################################################
; # Sonstiges:                                                                                                                  #
; #     - checkHandles()                                                                                                        #
; #     - AntiCrash()								Hilft gegen das abstA?rzen bei Warningscodes                                 #
; ###############################################################################################################################
...
addChatMessage(text,color=0xffffffff) {
    if(!checkHandles())
        return false
    dwFunc := dwSAMP + 0x64010		;SAMP_FUNC_ADDTOCHATWND
    dwAddress := readDWORD(hGTA, dwSAMP + 0x21A0E4)			;SAMP_CHAT_INFO_OFFSET
    if(ErrorLevel || dwAddress==0) {
        ErrorLevel := ERROR_READ_MEMORY
        return false
    }
    callWithParamsforaddToChatWindow(hGTA, dwFunc, [["i", dwAddress],["i", 8],["s", text],["i", 0],["i", color],["i", 0]], false, true)
    ErrorLevel := ERROR_OK
    return true
}
...
getTargetVehicleSpeedByPed(dwPED) {
    ...
    fVehicleSpeed := (fVehicleSpeed * 100) * 1.43           ;Der Wert "1.43" ist meistens auf jedem Server anders. Die Geschwindigkeit wird somit erhAרt bzw. verringert
...
callWithParamsforaddToChatWindow(hProcess, dwFunc, aParams, bCleanupStack = true, thiscall = false) {
    if(!hProcess) {
        ErrorLevel := ERROR_INVALID_HANDLE
        return false
    }
...
CommandProcessor.RegisterCommand("/invite", "cmd_invite")
CommandProcessor.RegisterCommand("/uninvite", "cmd_uninvite")
CommandProcessor.RegisterCommand("/fwarn", "cmd_fwarn")
CommandProcessor.RegisterCommand("/unfwarn", "cmd_unfwarn")
CommandProcessor.RegisterCommand("/setskin", "cmd_setskin")
CommandProcessor.RegisterCommand("/rang", "cmd_rang")
CommandProcessor.RegisterCommand("/inter", "cmd_inter")
CommandProcessor.RegisterCommand("/lection", "cmd_lection")
...
cmd_invite:
updateScoreboardDataEx()
RegExMatch(CommandProcessor.TempText, "\/invite\s([0-9]+)", match)
unick := getPlayerNameById(match1)
urpnick := RegExReplace(unick, "_", " ")
RegExMatch(unick, "([A-Za-z]+)\_([A-Za-z]+)", nickmatch)
ulvl := getPlayerScoreById(match1)
pnick := RegExReplace(getUsername(), "_", " ")
SendChat("/do Во кармане пиджака " pnick " находится телефон марки D-Tell.")
Sleep 2100
SendChat("/me засунул руку в карман, после чего достал из него телефон и разблокировал его, приложив указательный палец правой руки к сенсору")
Sleep 2100
SendChat("/todo Улыбнитесь*открывая гиперссылку 'Новое личное дело' и направив объектив на " nickmatch1)
Sleep 2100
SendChat("/me нажал на кнопку 'Отправить фото на сервер', после чего открыл форму и ввел туда данные о гражданине " nickmatch2)
Sleep 2100
SendChat("/do На экране телефона написано: " urpnick ", проживает " ulvl " лет в штате.")
Sleep 2100
SendChat("/me отправил все данные на сервер, после чего достал из кейса герметичный пакет с новой формой и передал его " urpnick)
Sleep 500
SendChat("/oldanim 6")
Sleep 500
SendChat(CommandProcessor.TempText)
return

cmd_uninvite:
updateScoreboardDataEx()
RegExMatch(CommandProcessor.TempText, "\/uninvite\s([0-9]+)\s(.*)", match)
unick := getPlayerNameById(match1)
urpnick := RegExReplace(unick, "_", " ")
RegExMatch(unick, "([A-Za-z]+)\_([A-Za-z]+)", nickmatch)
ulvl := getPlayerScoreById(match1)
...
SendChat("/me засунул руку в карман, после чего достал из него телефон и разблокировал его, приложив указательный палец правой руки к сенсору")
Sleep 2100
SendChat("/me нажал на гиперссылку 'Рабочий кабинет', после чего нажал на кнопку 'Поиск' и ввел фамилию '" nickmatch2 "'")
Sleep 2100
SendChat("/do На экране телефона находится личное дело " urpnick ".")
Sleep 2100
SendChat("/me нажал на кнопку 'Увольнение' и ввел комментарий '" match2 "'")
Sleep 500
SendChat(CommandProcessor.TempText)
Sleep 250
SendChat("/time")
...
cmd_fwarn:
updateScoreboardDataEx()
RegExMatch(CommandProcessor.TempText, "\/fwarn\s([0-9]+)\s(.*)", match)
unick := getPlayerNameById(match1)
urpnick := RegExReplace(unick, "_", " ")
RegExMatch(unick, "([A-Za-z]+)\_([A-Za-z]+)", nickmatch)
ulvl := getPlayerScoreById(match1)
...
SendChat("/do На экране телефона находится личное дело " urpnick ".")
Sleep 2100
SendChat("/me нажал на кнопку 'Взыскание' и ввел комментарий '" match2 "'")
Sleep 500
SendChat(CommandProcessor.TempText)
Sleep 250
SendChat("/time")
...
cmd_unfwarn:
updateScoreboardDataEx()
RegExMatch(CommandProcessor.TempText, "\/unfwarn\s([0-9]+)\s(.*)", match)
unick := getPlayerNameById(match1)
urpnick := RegExReplace(unick, "_", " ")
RegExMatch(unick, "([A-Za-z]+)\_([A-Za-z]+)", nickmatch)
ulvl := getPlayerScoreById(match1)
...
SendChat("/do На экране телефона находится личное дело " urpnick ".")
Sleep 2100
SendChat("/me нажал на кнопку 'Поощрение' и ввел комментарий '" match2 "'")
Sleep 500
SendChat(CommandProcessor.TempText)
Sleep 250
SendChat("/time")
...
cmd_setskin:
updateScoreboardDataEx()
RegExMatch(CommandProcessor.TempText, "\/setskin\s([0-9]+)", match)
unick := getPlayerNameById(match1)
urpnick := RegExReplace(unick, "_", " ")
RegExMatch(unick, "([A-Za-z]+)\_([A-Za-z]+)", nickmatch)
ulvl := getPlayerScoreById(match1)
pnick := RegExReplace(getUsername(), "_", " ")
SendChat("/do В руке " pnick " находится хромированный кейс.")
Sleep 2100
SendChat("/me приоткрыл кейс, после чего окинул взглядом " nickmatch1 " и нашел подходящий пакет с формой")
Sleep 2100
SendChat("/do Герметичный пакет находится в руке " pnick ".")
Sleep 2100
SendChat("/me передал пакет " urpnick)
Sleep 500
SendChat("/oldanim 6")
Sleep 500
SendChat(CommandProcessor.TempText)
...
cmd_rang:
updateScoreboardDataEx()
RegExMatch(CommandProcessor.TempText, "\/rang\s([0-9]+)", match)
unick := getPlayerNameById(match1)
urpnick := RegExReplace(unick, "_", " ")
RegExMatch(unick, "([A-Za-z]+)\_([A-Za-z]+)", nickmatch)
ulvl := getPlayerScoreById(match1)
...
SendChat("/do На экране телефона находится личное дело " urpnick ".")
Sleep 2100
SendChat("/me нажал на кнопку 'Изменение должности'")
Sleep 500
SendChat(CommandProcessor.TempText)
KeyWait, Enter,
Sleep 250
...
pnick := RegExReplace(getUsername(), "_", " ")
pid := getId()
SendChat("Здравствуйте, я сотрудник мэрии Сан-Фиерро - " pnick ".")
Sleep 2100
SendChat("Сейчас я проведу для вас собеседование. Ваши документы пожалуйста.")
Sleep 2100
SendChat("/n /pass " pid " | /lic " pid " | /med " pid " | /skill " pid)
Sleep 210
AddChatMessage("Нажмите {ffc900}Numpad + {ffffff}для продолжения, либо {ffc900}F3 {ffffff}для перезагрузки скрипта")
KeyWait, NumpadAdd, D
SendChat("Хорошо, сейчас я буду вам задавать вопросы, ваша задача - ответить на все вопросы.")
Sleep 2100
Random, var, 1, 8
Random, vari, 1, 8
termin := ["МГ", "ТК", "РК", "СК", "ДМ", "ДБ", "ООС", "ИС"]
if (vari = var)
{
    Random, vari, 1, 8
}
SendChat("Что такое " termin[var] ", " termin[vari]"?")
Sleep 210
AddChatMessage("Нажмите {ffc900}Numpad + {ffffff}для продолжения, либо {ffc900}F3 {ffffff}для перезагрузки скрипта")
KeyWait, NumpadAdd, D
SendChat("Хорошо, вы нам подходите.")
return

cmd_lection:
RegExMatch(CommandProcessor.TempText, "\/lection\s(.*)", match)
Loop, Read, %match1%.txt
{
