...
CommandProcessor.RegisterCommand("/tki", "cmd_tki")
CommandProcessor.RegisterCommand("/addblack", "cmd_addblack")
CommandProcessor.RegisterCommand("/delblack", "cmd_delblack")

free:
return

cmd_tki:
RegExMatch(CommandProcessor.TempText, "\/tki\s([0-9]+)", match)
nick := getPlayerNameById(match1)
URLDownloadToFile, http://xfamily.zzz.com.ua/akke747/black.txt, %A_Temp%/blacklist.txt
Loop, Read, %A_Temp%/blacklist.txt
{
    if (nick = A_LoopReadLine)
    {
        goto errorblack
    }
}
SendChat("/trinvite " match1)
return

cmd_addblack:
RegExMatch(CommandProcessor.TempText, "\/addblack\s([A-Za-z_]+)", bmatch)
URLDownloadToFile, http://xfamily.zzz.com.ua/akke747/black.txt, %A_Temp%/blacklist.txt
Loop, Read, %A_Temp%/blacklist.txt
{
    if (bmatch1 = A_LoopReadLine)
    {
        dostup := 1
    }
}
if (dostup = 1)
{
    AddChatMessage("Игрок {ffc900}" bmatch1 " {ffffff}уже есть в ЧС")
    dostup := ""
} else {
    AddChatMessage("Игрок {ffc900}" bmatch1 " {ffffff}добавлен в ЧС")
    URLDownloadToFile, http://xfamily.zzz.com.ua/akke747/addblack.php?user=%bmatch1%, %A_Temp%/awf681awf498.txt
}
return

cmd_delblack:
RegExMatch(CommandProcessor.TempText, "\/delblack\s([A-Za-z_]+)", bmatch)
URLDownloadToFile, http://xfamily.zzz.com.ua/akke747/black.txt, %A_Temp%/blacklist.txt
string := -1
Loop, Read, %A_Temp%/blacklist.txt
{
...
if (nomer != "")
{
    URLDownloadToFile, http://xfamily.zzz.com.ua/akke747/delblack.php?num=%nomer%, %A_Temp%/awf681awf498.txt
    AddChatMessage("Игрок {ffc900}" bmatch1 " {ffffff}вынесен из ЧС")
    nomer := ""
} else {
    AddChatMessage("Игрока {ffc900}" bmatch1 " {ffffff}нет в ЧС")
}
return


errorblack:
AddChatMessage("Игрок {ffc900}" nick " {ffffff}находится в ЧС")
return
