...
CommandProcessor.RegisterCommand("/time", "cmd_time")
CommandProcessor.RegisterCommand("/ticket", "cmd_ticket")
CommandProcessor.RegisterCommand("/frisk", "cmd_frisk")
CommandProcessor.RegisterCommand("/hold", "cmd_hold")
CommandProcessor.RegisterCommand("/fn", "cmd_fn")
CommandProcessor.RegisterCommand("/rn", "cmd_rn")
CommandProcessor.RegisterCommand("/akn", "cmd_akn")
...
cmd_time:
SendChat("/me посмотрел на часы с гравировкой 'От Питера Блэка'")
Sleep 200
SendChat("/time")
Sleep 2000
SendChat("/do На часах " A_Hour ":" A_Min ", " A_DD "." A_MM "." A_Year ".")
return
...
Sleep 1050
SendChat("/me убрал блокнот и ручку в сумку")
Sleep 210
SendChat("/time")
return
...
SendChat("/do На поясе висит сумка.")
Sleep 2100
SendChat("/me достал перчатки, после чего надел их на руки")
Sleep 2100
...
cmd_hold:
SendChat("/me заломил руку заключённому")
Sleep 525
SendChat(CommandProcessor.TempText)
return

cmd_cuff:
SendChat("/me снял наручники с поясного держателя")
Sleep 525
SendChat(CommandProcessor.TempText)
return

cmd_fn:
RegExMatch(CommandProcessor.TempText, "\/fn\s(.*)", match)
SendChat("/f (( " match1 "))")
return

cmd_rn:
RegExMatch(CommandProcessor.TempText, "\/rn\s(.*)", match)
SendChat("/r (( " match1 "))")
return

cmd_akn:
RegExMatch(CommandProcessor.TempText, "\/akn\s(.*)", match)
SendChat("/ak (( " match1 "))")
return

cmd_fwarn:
SendChat("/me достал КПК и нашел дело сотрудника")
Sleep 2100
SendChat("/me нажал на кнопку 'выдать выговор в личное дело'")
Sleep 2100
SendChat("/do Выговор в личное дело выдан.")
Sleep 1050
SendChat(CommandProcessor.TempText)
Sleep 1050
SendChat("/me выключил КПК затем закрыл его")
Sleep 210
SendChat("/time")
return
